...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.upsample_embedding = nn.Upsample(scale_factor=4,
    ...
    def forward(self, x):
        ...
        # embedding vector
        embedding = xi[-1]
        embedding = self.upsample_embedding(embedding)

        # Up path
        for i, layer in enumerate(self.layers[self.num_layers:-1]):
            xi[-1] = layer(xi[-1], xi[-2 - i])

        return self.layers[-1](xi[-1]), embedding
        #return self.layers[-1](xi[-1]), xi[-1]
